...
class CamRestEvaluator(GenericEvaluator):
    ...
    def run_metrics(self):
        ...
        # eng
        # raw_json = open('./data/CamRest676/CamRest/CamRest676.json')
        # raw_entities = open('./data/CamRest676/CamRest/CamRestOTGY.json')
...
class KvretEvaluator(GenericEvaluator):
    def __init__(self, result_path):
        ...
        self.raw_data = []
        test_json_path = ['./data/kvret/smd/kvret_test_public.json', './data/kvret/indosmd/IndoSMD_test.json']
        for file in test_json_path:
            raw_json = open(file)
            self.raw_data.extend(json.loads(raw_json.read().lower()))
            raw_json.close()
